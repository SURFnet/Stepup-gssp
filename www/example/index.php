...
$app->get('/login', function (Request $request) use ($app, $options) {
    ...
    $app['session']->set('authn', array('returnURL' => $return));
...
$app->post('/login', function (Request $request) use ($app, $options) {
    $userId = $request->get('userID');
    $userId = preg_replace("/[^a-zA-Z0-9_-]+/", "", $userId);
    $authn = $app['session']->get('authn');
    $authn['username'] = $userId;
    $app['session']->set('authn', $authn);
    $return = $authn['returnURL'];
...
$app->post('/switch-locale', function (Request $request) use ($app, $options) {
    ...
    $locale = filter_var($request->get('switch_locale'), FILTER_VALIDATE_REGEXP, $opt);
